...
const lightingSlice = createSlice({
  ...
  reducers: {
    // 环境光配置 - 优化为直接修改状态
    setAmbientLight: (state, action: PayloadAction<Partial<AmbientLightConfig>>) => {
      Object.assign(state.config.ambient, action.payload);
      state.activePreset = null;
    },
    
    // 平行光配置
    setDirectionalLight: (state, action: PayloadAction<Partial<DirectionalLightConfig>>) => {
      Object.assign(state.config.directional, action.payload);
      state.activePreset = null;
    },
    
    setDirectionalLightPosition: (state, action: PayloadAction<Partial<Vector3>>) => {
      Object.assign(state.config.directional.position, action.payload);
      state.activePreset = null;
    },
    
    // 半球光配置
    setHemisphereLight: (state, action: PayloadAction<Partial<HemisphereLightConfig>>) => {
      Object.assign(state.config.hemisphere, action.payload);
      state.activePreset = null;
    },
    
    // 点光源配置
    setPointLight: (state, action: PayloadAction<Partial<PointLightConfig>>) => {
    ...
    // 更新整体配置
    updateLightingConfig: (state, action: PayloadAction<Partial<LightingConfig>>) => {
    ...
    // 应用预设
    applyPreset: (state, action: PayloadAction<{ name: string; config: Partial<LightingConfig> }>) => {
