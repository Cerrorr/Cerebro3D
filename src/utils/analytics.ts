...
export class BaiduAnalytics {
  ...
  static init(): void {
    ...
    if (!analyticsId) {
      devLog.warn(
        '百度统计ID未配置，请在.env文件中设置VITE_BAIDU_ANALYTICS_ID'
      );
      return;
    }
  ...
  static trackPageView(pagePath: string, pageTitle?: string): void {
    ...
    try {
      ...
      devLog.log(
        `📊 页面访问追踪: ${pagePath}${pageTitle ? ` (${pageTitle})` : ''}`
      );
    } catch (error) {
  ...
  static trackEvent(
    ...
    try {
      ...
      devLog.log(
        `📊 事件追踪: ${category} -> ${action}${label ? ` (${label})` : ''}${value !== undefined ? ` [${value}]` : ''}`
      );
    } catch (error) {
